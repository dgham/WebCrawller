            ...
            foreach ($arr as $item) {
                ...
                $brower->get($href)
                    ->then(function (ResponseInterface $response) {
                        ...
                        $speciality = trim(Utf8_ansi(utf8_decode($crawler->filter('.docsubtitle')->text())));
                        $speciality = strtoupper($speciality);
                        $speciality = str_replace("'", '', $speciality);
                        $speciality = str_replace(",", '', $speciality);
                        $address = strtolower(trim(Utf8_ansi(utf8_decode($crawler->filter('.pf-itempage-sidebarinfo-elurl.pf-itempage-sidebarinfo-elitem')->text()))));
                        ...
                        $exercie = str_replace("'", "`", $exercie);
                        $exercie = utf8_decode($exercie);
                        $bio = trim(utf8_decode(Utf8_ansi($crawler->filter('.pfdetailitem-subelement.pf-onlyitem.clearfix .clearfix')->text())));
                        $bio = str_replace("'", '', $bio);
                        $bio = str_replace(",", '', $bio);
                        ...
                        try {
                            ...
                            if ($sql->rowCount() > 0) {
                                ...
                                $sql = "INSERT INTO wp_doctor (id_doctor, doctor_name,doctor_speciality,doctor_bio,doctor_address,doctor_country,doctor_location,
                                                doctor_phone,exercie_mode,created_at,picture) VALUES (0,'$name','$speciality','$bio','$address','$country','$location','$phone','$exercie',now(),'$picture')";
                                $conn->exec($sql);
                                echo '#';
                                $fetch = $conn->prepare("SELECT * from wp_doctor where doctor_name LIKE '%$name%' and doctor_address like '%$address%'");
                                $fetch->execute();
                                if ($fetch->rowCount() > 0) {
                                    $conn = new PDO("mysql:host=$servername;dbname=$dbname;charset=utf8mb4", $username, $password);
                                    $result = $fetch->fetchAll(); //make the select
                                    $idDoctor = $result[0]['id_doctor'];
                                    ...
                                    for ($i = 0; $i < sizeof($day); $i++) {
                                        $morningStart = str_replace("\t", "", substr($time[$i], 1, 5));
                                        $morningStart = str_replace(" ", "", $morningStart);
                                        $morningStart = str_replace(",", "", $morningStart);
                                        $morningClose = str_replace("\t", "", substr($time[$i], 6, 10));
                                        $morningClose = str_replace("\n", "", $morningClose);
                                        $morningClose = str_replace(",", "", $morningClose);
                                        $morningClose = str_replace("    ", "", $morningClose);
                                        $eveningStart = str_replace("\t", "", substr($time[$i], 55, -5));
                                        $eveningStart = str_replace(" ", "", $eveningStart);
                                        $eveningStart = str_replace("\n", "", $eveningStart);
                                        $eveningStart = str_replace(",", "", $eveningStart);
                                        $eveningClose = str_replace("\t", "", substr($time[$i], -5, 5));
                                        $eveningClose = str_replace(" ", "", $eveningClose);
                                        $eveningClose = str_replace(",", "", $eveningClose);
                                        $eveningClose = str_replace("\n", "", $eveningClose);
                                        $conn = new PDO("mysql:host=$servername;dbname=$dbname;charset=utf8mb4", $username, $password);
                                        $workTime = "INSERT INTO wp_work_time (id, day,morning_start,morning_close,evening_start,evening_close,id_doctor,created_at) 
                                       VALUES (0,'$day[$i]','$morningStart','$morningClose','$eveningStart','$eveningClose',$idDoctor,now())";
                                        $conn->exec($workTime);
                                    }
                                }
                            }
